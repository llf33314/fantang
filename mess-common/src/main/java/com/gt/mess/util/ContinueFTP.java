...
public class ContinueFTP {
     ...
     * @throws IOException
     */
    public Map< String,Object > download( String local , FtpProperties ftpProperties ) throws Exception {
        connect( ftpProperties.getStaticSourceFtpIp(), CommonUtil.toInteger( ftpProperties.getStaticSourceFtpPort() ), ftpProperties.getStaticSourceFtpUser(),
                        ftpProperties.getStaticSourceFtpPwd() );
        Boolean is_connect = connect( ftpProperties.getStaticSourceFtpIp(), CommonUtil.toInteger( ftpProperties.getStaticSourceFtpPort() ), ftpProperties.getStaticSourceFtpUser(),
                        ftpProperties.getStaticSourceFtpPwd() );
                ...
                try {
                    ...
                    while ( ( c = in.read( bytes ) ) != -1 ) {
                        ...
                        if ( nowProcess > process ) {
                            process = nowProcess;
                            if ( process % 10 == 0 ) logger.info( "下载进度：" + process );
     ...
     * @throws IOException
     */
    public UploadStatus upload( String local, String remote , FtpProperties ftpProperties ) throws Exception {
        UploadStatus result;
        try {
            Boolean is_connect = connect( ftpProperties.getStaticSourceFtpIp(), CommonUtil.toInteger( ftpProperties.getStaticSourceFtpPort() ),
                            ftpProperties.getStaticSourceFtpUser(), ftpProperties.getStaticSourceFtpPwd() );
     ...
     * @throws IOException
     */
    public Map< String,Object > upload( String local , FtpProperties ftpProperties) throws Exception {
        Map< String,Object > result = new HashMap< String,Object >();
        String remote = local.split( "upload" )[1];
        UploadStatus resultStr = upload( local, remote , ftpProperties);
        String msg = "";
        result.put( "code", -1 );
        switch ( resultStr ) {
