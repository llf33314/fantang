...
public final class QRcodeKit {
	...
	public static String buildQRcode(String contents,
			String path,
			Integer width,
			Integer height,FtpProperties ftpProperties) {
		...
		try {
			...
			writeToFile(bitMatrix, "jpg", file, ftpProperties);
	...
	public static String buildQRcode(String contents,
			...
			Integer width,
			Integer height,
			String logoPath,FtpProperties ftpProperties) {
		MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
		// 用于设置QR二维码参数
		Map<EncodeHintType, Object> hints = new HashMap<EncodeHintType, Object>();
		// 设置编码方式
		...
		try {
			BitMatrix bitMatrix = multiFormatWriter.encode(contents,
					...
					hints);
			file = new File(path, System.currentTimeMillis() + ".jpg");
			writeToFile(bitMatrix, "jpg", file, new File(logoPath),ftpProperties);
		} catch (WriterException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
	...
	public static void writeToFile(BitMatrix matrix,
			String format,
			File file,FtpProperties ftpProperties) throws IOException {
	...
	public static void writeToFile(BitMatrix matrix,
			String format,
			File file,
			File logoImg,FtpProperties ftpProperties) throws IOException {
		BufferedImage image = toBufferedImage(matrix);
		// 构建绘图对象
